...
func _physics_process(delta: float) -> void:
	...
	# Manejar salto con la tecla W
	if Input.is_action_just_pressed("jump_p1") and is_on_floor():  # Salto con W
		velocity.y = JUMP_VELOCITY

	# Manejar movimiento con las teclas A y D
	var direction = 0
	if Input.is_action_pressed("move_left_p1"):  # Moverse a la izquierda con A
		direction = -1
	elif Input.is_action_pressed("move_right_p1"):  # Moverse a la derecha con D
		direction = 1
