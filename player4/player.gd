...
func _physics_process(delta: float) -> void:
	...
	# Manejar salto con la flecha arriba
	if Input.is_action_just_pressed("ui_up") and is_on_floor():  # Salto con Flecha Arriba
		velocity.y = JUMP_VELOCITY

	# Manejar movimiento a la izquierda y derecha con las flechas
	var direction = 0
	if Input.is_action_pressed("ui_left"):  # Moverse a la izquierda con Flecha Izquierda
		direction = -1
	elif Input.is_action_pressed("ui_right"):  # Moverse a la derecha con Flecha Derecha
		direction = 1
...
func _on_body_entered(body):
	# Esta función se ejecuta cuando otro cuerpo entra en el área de colisión de player 4
	...
	if body.is_in_group("Players2"):  # Verificar que el cuerpo colisionado esté en el grupo "Player"
		print(self.name + " choco con " + body.name)
